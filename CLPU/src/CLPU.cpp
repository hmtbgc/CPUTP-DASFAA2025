...
int dw_n_walks = 10; // DeepWalk parameter "\gamma" = walks per vertex
...
void compute_node_freq() {
  node_cnt = static_cast<int *>(
      aligned_malloc((nv + 1) * sizeof(int), DEFAULT_ALIGN));
#pragma omp parallel for num_threads(n_threads)
  for (int i = 0; i < nv * dw_n_walks; i++) {
...
#pragma omp atomic
    node_cnt[lastnode]++;
    for (int j = 2; j < dw_walk_length; j++) {
      if (degree[lastnode] == 0)
        break;
      lastnode = sample_neighbor(lastnode);
#pragma omp atomic
      node_cnt[lastnode]++;
    }
  }
...
void init_hsm(
  ...
  sort(idx.begin(), idx.end(), // we need to sort the index array as in probs
       [&probs](size_t i1, size_t i2) { return probs[i1] > probs[i2]; }); // large -> small
  if (verbosity > 1)
    cout << "." << flush;
...
inline void update_new(float *w_s, float *w_t, float *w_t_cache, float lr, 
                      const int num, const int labelsum) {
  ...
  score = (labelsum - fast_sigmoid(score) * num) * lr;
  AVX_LOOP
  for (int c = 0; c < n_hidden; c++) {
...
inline void update( // update the embedding, putting w_t gradient in w_t_cache
  ...
  //cout << chrono::duration_cast<chrono::duration<float>>(end_ - begin_).count() << endl;
  // #pragma omp atomic
  // update_time += chrono::duration_cast<chrono::duration<float>>(end_ - begin_).count();
}
...
void Train() {
  ...
  const float subsample = 1e-3 * nv * dw_n_walks * dw_walk_length;
#pragma omp parallel num_threads(n_threads)
  {
    ...
    unordered_map<int, int> labelsum_map;

#pragma omp barrier
    while (true) {

      //auto begin_0 = chrono::steady_clock::now();
...
#pragma omp atomic
      ...
      // auto end_0 = chrono::steady_clock::now();
      // cout << "0:" << chrono::duration_cast<chrono::duration<float>>(end_0 - begin_0).count()*1000 << endl; 
      ...
      // auto begin_time0 = chrono::steady_clock::now();
      for (int i = 1; i < dw_walk_length; i++)
        dw_rw[i] =
            sample_neighbor(dw_rw[i - 1]); // sample random walk from the source
      // auto end_time0 = chrono::steady_clock::now();

      // cout << "1:" << chrono::duration_cast<chrono::duration<float>>(end_time0 - begin_time0).count()*1000 << endl;
      // #pragma omp atomic
      // time_0 += chrono::duration_cast<chrono::duration<float>>(end_time0 - begin_time0).count();

      // auto begin_time1 = chrono::steady_clock::now();
      ...
      for (int dwi = 0; dwi < dw_walk_length; dwi++) {
        ...
        if (n1 == -1)
          break;          
        // if ((sqrt(node_cnt[n1] / subsample) + 1) * subsample / node_cnt[n1] <
...
// #endif
    ...
    //       int thread_num = hsm_indptrs[n1 + 1] - hsm_indptrs[n1];
    // #pragma omp parallel num_threads(thread_num)

          size_t end = hsm_indptrs[n2 + 1];
          size_t base = end - lastnum; 
          size_t begin = std::max(base, hsm_indptrs[n2]);

          // size_t begin = hsm_indptrs[n2];
          // size_t end = hsm_indptrs[n2 + 1];

          for (size_t hsi = begin; hsi < end; hsi++) {
...
// #else
                hsm_codes[hsi];
// #endif
            labelsum_map[tou] += lab;
            num_map[tou] += 1;

            //update(&wCtx[tou * n_hidden], &wVtx[n1 * n_hidden], cache, lr, lab);
          }
        }     
        

          for (auto& x : labelsum_map) {
            int _tou = x.first;
            int _labelsum = labelsum_map[_tou];
            int _num = num_map[_tou];
            update_new(&wCtx[_tou * n_hidden], &wVtx[n1 * n_hidden], cache, lr, _num, _labelsum);
          }

          AVX_LOOP
          for (int c = 0; c < n_hidden; c++) // update cache of \Phi[n2]
            wVtx[n1 * n_hidden + c] += cache[c];
      }


      // auto end_time1 = chrono::steady_clock::now();
      // cout << "2:" << chrono::duration_cast<chrono::duration<float>>(end_time1 - begin_time1).count() * 1000 << endl;
...
int main(int argc, char **argv) {
  auto begin_total = chrono::steady_clock::now();
  auto begin_init = chrono::steady_clock::now();
  int a;
  string network_file, embedding_file, log_file;
  ...
  if ((a = ArgPos(const_cast<char *>("-log"), argc, argv)) > 0)
    log_file = argv[a + 1];
  else {
    if (verbosity > 0)
      cout << "Log file not given! Aborting now.." << endl;
    return 1;
  }
  ...
  if ((a = ArgPos(const_cast<char *>("-lastnum"), argc, argv)) > 0)
    lastnum = atoi(argv[a + 1]);
#if INIT_HSM == HSM_INIT_PR
  ...
  degree = static_cast<int *>(aligned_malloc(nv * sizeof(int), DEFAULT_ALIGN));

auto begin_estimate = chrono::steady_clock::now();
...
// auto end_estimate = chrono::steady_clock::now();
// cout << "estimate time:" << chrono::duration_cast<chrono::duration<float>>(end_estimate - begin_estimate).count() 
// << " s" << endl;
...
auto end_init = chrono::steady_clock::now();
  ...
  auto begin_init_hsm = chrono::steady_clock::now();

  init_hsm(hsm_weights);

  auto end_init_hsm = chrono::steady_clock::now();
//   cout << "init_hsm time:" << chrono::duration_cast<chrono::duration<float>>(end_init_hsm - begin_init_hsm).count() 
// << " s" << endl;

  //compute_node_freq();

  auto begin_train = chrono::steady_clock::now();
  Train();
  ...
  ofstream output(embedding_file, ofstream::out);
  output << nv << endl;
  for (int i = 0; i < nv; ++i) {
      output << i;
      for (int j = 0; j < n_hidden; ++j) {
          output << " " << wVtx[i * n_hidden + j];
      }
      output << endl;
  }
  ...
  ofstream logout(log_file, ofstream::out);
  auto end_total = chrono::steady_clock::now();

  logout << "Average code size: " << hsm_indptrs[nv] / float(nv) << endl;
  logout << "total time used: " << chrono::duration_cast<chrono::duration<float>>(end_total - begin_total).count() << "s" << endl;
  logout << "init time used: " << chrono::duration_cast<chrono::duration<float>>(end_init - begin_init).count() << "s" << endl;
  logout << "build HSM time used: " << chrono::duration_cast<chrono::duration<float>>(end_init_hsm - begin_init_hsm).count() << "s" << endl;
  logout << "train time used: " << chrono::duration_cast<chrono::duration<float>>(end_train - begin_train).count() << "s" << endl;
  logout.close();
  // cout << num << endl;
